...
class YOLODataset(BaseDataset):
    ...
    def build_transforms(self, hyp=None):
        ...
        paired = getattr(hyp, 'ch', 3) > 3
        if self.augment:
            hyp.mosaic = hyp.mosaic if self.augment and not self.rect else 0.0
            hyp.mixup = hyp.mixup if self.augment and not self.rect else 0.0
            transforms = v8_Pairedtransforms(self, self.imgsz, hyp) if paired else v8_transforms(self, self.imgsz, hyp)
        else:
            transforms = Compose([
                PairedLetterBox(new_shape=(self.imgsz, self.imgsz), scaleup=False)
                if paired else LetterBox(new_shape=(self.imgsz, self.imgsz), scaleup=False)
            ])
        transforms.append(
