...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        """Create EMA."""
        source_model = de_parallel(model)
        try:
            self.ema = deepcopy(source_model).eval()  # FP32 EMA
        except RuntimeError as e:
            # Some custom modules stash forward-pass tensors (for distillation/debugging) on the module itself.
            # If those tensors are graph-connected, PyTorch cannot deepcopy the model for EMA initialization.
            if 'graph leaves' not in str(e):
                raise
            for module in source_model.modules():
                if hasattr(module, 'kd_cache'):
                    module.kd_cache = {}
            self.ema = deepcopy(source_model).eval()
