#!/usr/bin/env python3
"""Train the dual-stream HGNetv2 HBB student with switchable HBB teacher backbones."""
...
DEFAULT_MODEL_PATH = REPO_ROOT / "model_yaml_hbb" / "yolov8-EntropyOffsetGate-deimhgnetv2-b0.yaml"
DEFAULT_DATA_PATH = REPO_ROOT / "data" / "LLVIP_hbb_student_external.yaml"
DEFAULT_SAVE_DIR = REPO_ROOT / "runs" / "detect" / "semd_student_dist_deimhgnetv2_hbb"
DEFAULT_TEACHER_CHECKPOINTS = {
    "hgnetv2": {
        "rgb": REPO_ROOT / "weight" / "teacher_hgnetv2_hbb_rgb" / "best.pt",
        "ir": REPO_ROOT / "weight" / "teacher_hgnetv2_hbb_ir" / "best.pt",
    },
    "yolov8": {
        "rgb": REPO_ROOT / "weight" / "teacher_yolov8_hbb_rgb" / "best.pt",
        "ir": REPO_ROOT / "weight" / "teacher_yolov8_hbb_ir" / "best.pt",
    },
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the dual-stream HGNetv2 HBB student with multi-teacher distillation."
    )
    ...
    parser.add_argument("--distill-disable-all", action=BooleanOptionalAction, default=False,
                        help=argparse.SUPPRESS)
    parser.add_argument("--distill-cross-attention", action=BooleanOptionalAction, default=True,
                        help=argparse.SUPPRESS)
    parser.add_argument("--distill-normal-distillation", action=BooleanOptionalAction, default=True,
                        help=argparse.SUPPRESS)
    parser.add_argument("--distill-only-normal", action=BooleanOptionalAction, default=False,
                        help="Strictly keep only same-modality branch KD (RGB<-RGB, IR<-IR).")
    parser.add_argument("--distill-disable-gate-kd", action=BooleanOptionalAction, default=False,
                        help="Disable gate KD while keeping the rest of the multimodal KD path active.")
    parser.add_argument("--distill-disable-pseudo-fusion-kd", action=BooleanOptionalAction, default=False,
                        help="Disable pseudo fusion feature KD and keep gate KD only.")
    parser.add_argument("--distill-head-kd-policy", type=str, default="off",
    ...
    parser.add_argument("--distill-schedule-enable", action=BooleanOptionalAction, default=False,
    ...
    parser.add_argument("--distill-student-entropy-weight-enable", action=BooleanOptionalAction, default=False,
    ...
    parser.add_argument("--distill-student-entropy-weight-detach", action=BooleanOptionalAction, default=True,
    ...
    parser.add_argument("--distill-stage2-cls-only", action=BooleanOptionalAction, default=False)
    parser.add_argument("--distill-resume-ckpt", type=str, default=None)
    parser.add_argument("--freeze-backbone-fusion", action=BooleanOptionalAction, default=False)
    parser.add_argument("--train-neck-head-only", action=BooleanOptionalAction, default=False)
    parser.add_argument("--stage2-lr-mult", type=float, default=0.1)
    parser.add_argument("--stage2-epochs", type=int, default=25)
    parser.add_argument("--stage2-disable-early-stop", action=BooleanOptionalAction, default=True)
    ...
    parser.add_argument("--audit-mode", action=BooleanOptionalAction, default=False)
    parser.add_argument("--audit-disable-early-stop", action=BooleanOptionalAction, default=True)
    ...
    parser.add_argument("--smoke-test", action=BooleanOptionalAction, default=False,
                        help="Load the model, read one batch, and run a single forward pass without training.")
    parser.add_argument("--smoke-test-split", type=str, default="val", choices=("train", "val", "test"))
    parser.add_argument("--smoke-test-batch", type=int, default=1)
    parser.add_argument("--smoke-test-workers", type=int, default=0)
    parser.add_argument("--online", action=BooleanOptionalAction, default=False)
    parser.add_argument("--augment", action=BooleanOptionalAction, default=True)
    parser.add_argument("--amp", action=BooleanOptionalAction, default=False)
    parser.add_argument("--rect", action=BooleanOptionalAction, default=False)
    parser.add_argument("--exist-ok", action=BooleanOptionalAction, default=True)
...
def validate_hbb_dataset(data_yaml_path):
    ...
    for split in ("train", "val"):
        ...
        if sample_line is None:
            continue
        if len(sample_line.split()) != 5:
            raise ValueError(
                f"dataset is not HBB-ready: {label_path} should have exactly 5 columns "
                f"(class + xywh), got {len(sample_line.split())}."
            )
...
def validate_layer_types(model, layer_ids, expected_type, side):
    ...
    mismatches = [idx for idx in layer_ids if layer_types[idx] != expected_type]
    if mismatches:
        raise RuntimeError(
            f"{side} layer taps {mismatches} are not {expected_type}. "
...
def build_student_probe_model(model_path, nc):
    ...
    return DetectionModel(str(model_path), ch=6, nc=nc, verbose=False), model_path


def resolve_student_taps(model_path):
    model_name = resolve_path(model_path).name.lower()
    if "-p2-" in model_name or "-p2p4-" in model_name:
...
def resolve_student_distill_config(model_path, nc, imgsz=640):
    ...
    validate_layer_types(student_model, fusion_layers, "EntropyOffsetGateFusion", "student fusion")
...
def run_smoke_test(overrides, data_yaml_path, model_path, split, batch_size, workers):
    from ultralytics.data.utils import check_det_dataset
    from ultralytics.models.yolo.detect import DetectionTrainer

    trainer = DetectionTrainer(overrides=overrides)
    trainer.data = check_det_dataset(str(data_yaml_path))
    if split not in trainer.data or not trainer.data[split]:
        raise ValueError(f"split '{split}' is not defined in dataset yaml: {data_yaml_path}")
    trainer.model = trainer.get_model(cfg=str(model_path), verbose=False)
    trainer.model = trainer.model.to(trainer.device)
    trainer.set_model_attributes()
    batch = next(iter(trainer.get_dataloader(trainer.data[split], batch_size=batch_size, rank=-1, mode="val")))
    ...
    if isinstance(preds, tuple):
        main_pred = preds[0]
        pred_shape = tuple(main_pred.shape) if hasattr(main_pred, "shape") else str(type(main_pred))
    else:
        pred_shape = tuple(preds.shape) if hasattr(preds, "shape") else str(type(preds))
    print(f"[smoke] split={split} batch={batch['img'].shape[0]} img={tuple(batch['img'].shape)} pred={pred_shape}")


def main():
    args = parse_args()
    data_yaml_path, data_cfg, dataset_root = validate_hbb_dataset(args.data)
    ...
    overrides = dict(
        ...
        distill_stage2_cls_only=bool(args.distill_stage2_cls_only),
        distill_resume_ckpt=str(stage2_resume_ckpt) if stage2_resume_ckpt else None,
        freeze_backbone_fusion=bool(args.freeze_backbone_fusion),
        train_neck_head_only=bool(args.train_neck_head_only),
        stage2_lr_mult=args.stage2_lr_mult,
        stage2_epochs=args.stage2_epochs,
        stage2_disable_early_stop=bool(args.stage2_disable_early_stop),
        ...
        audit_disable_early_stop=bool(args.audit_disable_early_stop),
        ...
        Distillation=None if distill_disabled else "MultiDistillation",
        loss_type=args.loss_type,
        distill_weight=0.0 if distill_disabled else args.distill_weight,
        Teacher_Model_RGB=None if distill_disabled else teacher_rgb_model,
        Teacher_Model_IR=None if distill_disabled else teacher_ir_model,
        Teacher_Model_RGB_Path=None if distill_disabled or teacher_rgb_path is None else str(teacher_rgb_path),
        Teacher_Model_IR_Path=None if distill_disabled or teacher_ir_path is None else str(teacher_ir_path),
        ...
        save_period=effective_save_period,
    )
    ...
    if args.smoke_test:
        ...
        smoke_overrides.update(
            batch=args.smoke_test_batch,
            workers=args.smoke_test_workers,
            device=args.device,
            augment=False,
            rect=False,
            save=False,
        ...
        run_smoke_test(
            ...
            split=args.smoke_test_split,
            batch_size=args.smoke_test_batch,
            workers=args.smoke_test_workers,
        )
        return
    ...
    trainer = DetectionTrainer(overrides=overrides)
    trainer.distill_entry_script = str(Path(__file__).resolve())
    trainer.train()
